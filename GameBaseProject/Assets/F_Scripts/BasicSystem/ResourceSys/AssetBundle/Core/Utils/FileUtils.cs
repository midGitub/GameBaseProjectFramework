...
public class FileUtils
{
    public static VersionInfo GetCurrentVerNo()
    {
        ...
        try
        {
            //获取当前版本，先检查persistentDataPath，然后检查streamingAssetsPath
            string localVerFileUrl = BundleConfig.Instance.BundlesPathForPersist + BundleConfig.Instance.VersionFileName;
            string fileInfo = null;
            if (File.Exists(localVerFileUrl))
            {
                fileInfo = File.ReadAllText(localVerFileUrl);
            }
            else
            {
                localVerFileUrl = string.Format("AssetBundles/{0}/{1}", BundleConfig.Instance.BundlePlatformStr, BundleConfig.Instance.VersionFileName);

                Stream stream = StreamingAssetLoad.GetFile(localVerFileUrl);
                if (stream == null)
                {
                    UnityEngine.Debug.LogError("Error 本地版本文件不存在");
                }
                else
                {
                    StreamReader st = new StreamReader(stream);
                    fileInfo = st.ReadToEnd();

                    st.Close();
                    stream.Close();
                }
            }
            curVerInfo = LitJson.JsonMapper.ToObject<VersionInfo>(fileInfo);
        }
    ...
    public static ChannelInfo GetCurrentChannelNo()
    {
        ChannelInfo curChannelInfo = null;
        try
        {
            string localVerFileUrl = string.Format("AssetBundles/{0}/{1}", BundleConfig.Instance.BundlePlatformStr, BundleConfig.Instance.ChannelFileName);
            Stream stream = StreamingAssetLoad.GetFile(localVerFileUrl);
            if (stream == null)
            {
                UnityEngine.Debug.LogError("Error 本地渠道文件不存在");
            }
            else
            {
                StreamReader st = new StreamReader(stream);
                string fileInfo = st.ReadToEnd();

                st.Close();
                stream.Close();

                curChannelInfo = LitJson.JsonMapper.ToObject<ChannelInfo>(fileInfo);
            }
        }
        catch { LuaInterface.Debugger.LogError("FileUtils: get current version fail !"); }
